...
def test_rewrite_score_dynamics_cli_writes_records_and_report(tmp_path: Path):
    ...
    completed = subprocess.run(
        [
            sys.executable,
            "-m",
            "scripts.rewrite_score_dynamics",
            str(records_path),
            "--output-records",
            str(output_path),
            "--report",
    ...
    assert rewritten[0].observations[1].score_delta == 0.3
    assert report["concept_usage"]
