...
def _load_cli_module():
    path = Path(__file__).resolve().parents[1] / "scripts" / "run_activation_trajectory_split_seed_sweep.py"
    spec = importlib.util.spec_from_file_location("run_activation_trajectory_split_seed_sweep_cli", path)
...
def test_run_activation_trajectory_split_seed_sweep_cli_writes_outputs(monkeypatch, tmp_path):
    cli = _load_cli_module()
    records = [_record(str(index), index % 2 == 0) for index in range(12)]
    records_path = tmp_path / "records.jsonl"
    features_path = tmp_path / "features.jsonl"
    output_json = tmp_path / "activation_split.json"
    output_csv = tmp_path / "activation_split.csv"
            ...
            rows.append(
                {
                    ...
                    "hidden_layer_mid_last_token": [value + 0.5 for value in vector],
                }
            )
    ...
    assert summary["feature_rows"]["feature_field"] == "hidden_layer_mid_last_token"
